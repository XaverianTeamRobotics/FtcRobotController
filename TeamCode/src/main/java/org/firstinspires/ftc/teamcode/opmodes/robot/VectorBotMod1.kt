...
class VectorBotMod1: BaseOpMode() {
    override fun construct() {
        ...
        addScript(ContinuousMotorScript(id = 5, input = {(gamepad2.right_stick_y).toDouble()})) //vertical lifter
        ...
        addScript(ContinuousServoScript(id = 2, input = {(gamepad2.right_trigger-gamepad2.left_trigger).toDouble()})) //claw rotator
        addScript(ContinuousServoScript(id = 3, input = ContinuousMotorScript.twoWayToggleInput(input = {gamepad2.x}))) //claw grabber
        addScript(ContinuousServoScript(id = 4, input = ContinuousMotorScript.threeWayInput({gamepad2.dpad_down}, {gamepad2.dpad_up}, {gamepad2.dpad_right}))) //intake
        addScript(ContinuousServoScript(id=5, input = {(gamepad1.right_trigger-gamepad1.left_trigger).toDouble()})) //screw jack
    }
